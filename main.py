...
# ======================================================
#   REENVÍO ROBUSTO CON REINTENTOS "INFINITOS"
# ======================================================
async def robust_forward_message(bot, chat_id, from_chat_id, message_id, max_retry_delay=60):
    """
    Reenvío con reintentos infinitos "inteligentes":
      - Reintenta en bucle para errores de red / timeout / flood / retry_after.
      - Corta definitivamente para errores no recuperables (Forbidden, mensaje inexistente, etc.).
      - Usa backoff exponencial hasta max_retry_delay segundos.
    Devuelve True si se reenvía, False si se abandona por error definitivo.
    """
    delay = 1
    attempt = 0
    ...
    while True:
        ...
        try:
            ...
            # Telegram indica explícitamente cuánto esperar
            sleep_for = int(getattr(e, "retry_after", delay))
            print(f"[robust_forward] RetryAfter para {message_id}, esperando {sleep_for}s")
        except (TimedOut, NetworkError) as e:
            # Problemas de red / timeout → reintentar
            print(f"[robust_forward] Error de red/timeout en {message_id}: {e}, reintentando en {delay}s")
            sleep_for = delay
        except Forbidden as e:
            # Usuario bloqueó al bot o no se puede enviar → no tiene sentido insistir
            print(f"[robust_forward] Forbidden para {message_id}: {e}, ABANDONO")
            return False
        except BadRequest as e:
            msg = str(e).lower()
            # Errores típicos no recuperables
            if "message to forward not found" in msg or "message_id_invalid" in msg:
                print(f"[robust_forward] BadRequest no recuperable en {message_id}: {e}, ABANDONO")
                return False
            # Otros BadRequest (momentáneos) → reintento
            print(f"[robust_forward] BadRequest recuperable en {message_id}: {e}, reintentando en {delay}s")
            sleep_for = delay
        except Exception as e:
            # Cualquier otra cosa rara → reintentar con backoff
            print(f"[robust_forward] Error inesperado en {message_id}: {e}, reintentando en {delay}s")
            sleep_for = delay

        # Esperar y aumentar delay hasta el máximo configurado
        await asyncio.sleep(sleep_for)
        delay = min(delay * 2, max_retry_delay)


# ======================================================
#   REENVÍO ORDENADO USANDO robust_forward_message
# ======================================================
async def send_topic(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for idx, mid in enumerate(mensajes, start=1):
        ok = await robust_forward_message(
            bot=bot,
            chat_id=user_id,
        ...
        if ok:
            ...
            # Mensaje imposible de reenviar: seguimos con el siguiente
            print(f"[send_topic] Mensaje {mid} saltado por error definitivo.")

        # Pequeña pausa anti-flood cada 70 mensajes reenviados con éxito
        if enviados > 0 and enviados % 70 == 0:
            print("[send_topic] Pausa anti-flood de 2s")
            await asyncio.sleep(2)

    await bot.send_message(
        chat_id=user_id,
        text=f"✔ Envío completado. {enviados} mensajes reenviados de {total} registrados 🎉",
        reply_markup=InlineKeyboardMarkup(
